...
def landproerty1():
    ...
    # con = dbConnection()
    # cursor = con.cursor()
    # cursor.execute("SELECT * FROM addland  admin = %s AND buyer  = %s ",(status,buyerstatus))
    # result1 = cursor.fetchall()
...
def status():
    ...
    con = dbConnection()
    cursor = con.cursor()
    cursor.execute("SELECT * FROM addland where admin = %s AND buyer  = %s ",(status,buyerstatus))
    result1 = cursor.fetchall()
